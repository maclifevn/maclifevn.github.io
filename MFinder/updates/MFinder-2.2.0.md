...
- Camera RAW files (CR3, NEF and others) no longer open in MFinder by default on a Mac without another RAW app — Preview.app stays the default and MFinder remains available in Open With.
- Search can now be switched off entirely in Settings › Search: the hotkey, the Search Files… menu items, Search Here… in Finder and the background indexer all stop, and switching it back on restores the hotkey you had recorded.
- New option in Settings › Overview to hide the MFinder icon from the menu bar. Reopen MFinder from Finder or Spotlight to reach Settings; the icon comes back on its own while a permission is missing or an upload is running.
- Menu Bar Icons on macOS 27 uses only the chevron as the boundary, so the divider no longer stays on the bar after icons are hidden.
- Hovering a row in the menu bar popover now shows a soft tint instead of a solid white block over a dark desktop.
...
- File RAW máy ảnh (CR3, NEF…) không còn mặc định mở bằng MFinder trên máy chưa có ứng dụng RAW khác — Preview.app vẫn là mặc định, MFinder vẫn có trong Open With.
- Có thể tắt hẳn Tìm kiếm trong Cài đặt › Tìm kiếm: phím tắt, các mục Search Files…, Search Here… trong Finder và trình lập chỉ mục nền đều dừng; bật lại thì phím tắt đã ghi được khôi phục.
- Tuỳ chọn mới trong Cài đặt › Tổng quan để ẩn icon MFinder khỏi thanh menu. Mở MFinder từ Finder hoặc Spotlight để vào Cài đặt; icon tự hiện lại khi thiếu quyền hoặc đang tải lên.
- Menu Bar Icons trên macOS 27 chỉ dùng mũi tên làm ranh giới, nên dấu gạch ngăn không còn kẹt lại trên thanh menu sau khi ẩn icon.
- Rê chuột lên một dòng trong popover ở thanh menu giờ hiện nền tô nhẹ thay vì ô trắng chói khi màn hình nền tối.
